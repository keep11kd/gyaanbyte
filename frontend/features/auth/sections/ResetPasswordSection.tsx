...
import { authBrand, authFeatures, resetPasswordContent } from "../data";
import type { ResetPasswordFormValues } from "../types";

export interface ResetPasswordSectionProps {
  /** Callback fired upon submitting new password values */
...
export default function ResetPasswordSection({
  ...
  hasValidToken = true,
  className = "",
}: Readonly<ResetPasswordSectionProps>) {
  return (
    <section className={`relative min-h-screen overflow-hidden bg-slate-950 ${className}`.trim()}>
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl items-center px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid w-full items-center gap-12 lg:grid-cols-2 lg:gap-16">
          {/* Left Side: Brand Context & Security Features */}
          <div className="hidden lg:block">
            <div className="max-w-xl">
              ...
              <p className="mt-6 text-lg leading-relaxed text-slate-400">
                Your new password should be unique, secure, and easy for you to
                remember. Protect your account with a strong password.
              </p>

              {/* Feature Highlights List */}
              <div className="mt-10 grid gap-4">
                {authFeatures.slice(1, 5).map((feature) => {
          ...
          {/* Right Side: Auth Card & Reset Form */}
          <div className="mx-auto w-full max-w-lg">
            <AuthCard>
              <AuthHeader content={resetPasswordContent} />
